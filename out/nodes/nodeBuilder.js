...
let NodeBuilder = class NodeBuilder {
    ...
    sortNodes(nodes) {
        let directoryNodes = nodes.filter(node => node instanceof types_4.DirectoryNode).map(node => node);
        let fileNodes = nodes.filter(node => node instanceof types_4.FileNode).map(node => node);
    ...
    sortDirectoryNodes(nodes) {
        const directoryNodes = nodes.filter(node => node instanceof types_4.DirectoryNode).map(node => node);
        if (this.platform.isWindows) {
    ...
    sortFileNodes(nodes) {
        const fileNodes = nodes.filter(node => node instanceof types_4.FileNode).map(node => node);
        if (this.platform.isWindows) {
